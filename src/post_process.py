...
class Post_Process:
    def __init__(self, width, length, layer_2_weight=0.5, layer_3_weight=0.2, length_weight=1.5):
        ...
        and the weights used for the Bresenham algorithm for scoring the lines'''
        self.width = width
        self.length = length
    ...
    def line_to_point_distance(self, x, y, a, b, c):
        '''Determine the distance from a cartesian point to a standard form line'''
        return (abs(a * x + b * y + c)) / (math.sqrt(a * a + b * b))

    def absolute_angle_to_line(self, l1):
        '''Determine the angle from the origin to the given line (in two-points format)'''
        x1, y1 = l1[0][0], l1[0][1]
        x2, y2 = l1[0][2], l1[0][3]
    ...
    def segments_intersect(self, l1, l2):
        '''Determine whether to line segments intersect (in two-points format)'''
    ...
    def lines_are_close(self, l1, l2, dist, theta_deg):
        ...
        Parameters:
            ...
            theta_deg (double): the maximum angle apart, in degrees, the lines must be
        
        Returns:
            bool: Whether the two provided lines are close, given the parameters
        '''
        ...
        shortest_distance = self.line_to_point_distance(x1, y1, a2, b2, c2)
        shortest_distance = self.line_to_point_distance(x2, y2, a2, b2, c2) if self.line_to_point_distance(x2, y2, a2, b2, c2) <= shortest_distance else shortest_distance
        shortest_distance = self.line_to_point_distance(x3, y3, a1, b1, c1) if self.line_to_point_distance(x3, y3, a1, b1, c1) <= shortest_distance else shortest_distance
        shortest_distance = self.line_to_point_distance(x4, y4, a1, b1, c1) if self.line_to_point_distance(x4, y4, a1, b1, c1) <= shortest_distance else shortest_distance

        return self.segments_intersect(l1, l2) or shortest_distance < dist or (abs(line_1_dist - line_2_dist) < dist and abs(line_1_angle - line_2_angle) < math.radians(theta_deg))

    def line_score(self, l1, pred_map):
        '''
        Determines a score for the line that represents how well the line fits the UNet output
        ...
        final_score = final_layer_1 + (final_layer_2 * self.layer_2_weight) + (final_layer_3 * self.layer_3_weight)
    ...
    def segment_image_to_robot(self, lines):
        '''
        Convert an array of lines in image pixel coordinates to robot coordinates

        Parameters:
            lines (array): an array of lines

        Returns:
            array of line.msg: an array of line message objects containing the converted points
        '''
        ...
        for l in lines:
            ...
            new_line.y2 = l[0][3] * scaling[0] + mins[0]
    ...
    def extract_lines(self, prediction_map):
        '''
        Determine, in two-points format, where there are clear lines in the image that represent rows
        ...
        prediction_gray = np.array(prediction_map[0].cpu() * 255, dtype=np.uint8).transpose(1, 2, 0)
