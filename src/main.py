...
from CMU_UNet_Node.msg import line_list
import std_msgs.msg
...
class Lidar_Process_Node:
    ...
    def velodyne_callback(self, data):
        '''Callback function for the Lidar, where
        point clouds are converted to lines and published'''
        ...
        cv.imwrite("/home/aaron/pcl.jpg", image_pcl)
        ...
        if lines is not None:
            for line in lines:
                cv.line(
                    prediction_image, (line[0][0], line[0][1]),
                    (line[0][2], line[0][3]), (0, 0, 255), 2, cv.LINE_AA)

        cv.imwrite("/home/aaron/lines.jpg", prediction_image)
        ...
        # Construct a line_list message to publish,
        # containing an array of line messages
